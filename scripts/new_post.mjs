...
const all = (k) => args.reduce((a, v, i) => (v === k && args[i + 1] ? [...a, args[i + 1]] : a), []);
const TAGS = all("--tag");
const TAGS_EN = all("--tag-en");
...
<main class="article-shell">
  ...
  <article class="article-card">
    ...
    <div class="post-tags" aria-label="ハッシュタグ">${TAG_PAIRS.map(([ja, en]) => `<a href="/tags/#tag-${esc(ja.replace(/^#/, ""))}"${en !== ja ? ` data-en="${esc(en)}"` : ""}>${esc(ja)}</a>`).join("")}</div>
    <div class="post-like">
